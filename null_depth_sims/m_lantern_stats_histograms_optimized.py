...
def compute_overlap_int(optics, f_number):
    ...
    ouput_wf_complex = prop_fibre_input_field(optics, np.zeros(zernike_indexes.shape))
...
plt.hist(
    injection_vals,
    alpha=0.5,
    label=f"Aberrated injection (tt rms = {tip_tilt_rms*1e9*np.sqrt(2)*4:.1f} nm)",  # *1e9 to convert to nm, *sqrt(2)*4 to convert to RMS/dlux things
)
